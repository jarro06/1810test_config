...
def job_config = [
        job: [
                name: "1810test_Pipeline_develop"
        ],
        git: [
                branch: "feature-1666089571262"
        ]
]
...
def customLib = library identifier: '1810test_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/jarro06/1810test_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.1810test.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                    jslGit.checkout( config, "JEQP", "1810test_Pipeline_develop", "feature-1666089571262")
                }
            }    
        }
        stage('Build') {
